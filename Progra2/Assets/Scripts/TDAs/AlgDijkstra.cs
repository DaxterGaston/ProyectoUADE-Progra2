...
public class AlgDijkstra
{
    ...
    public static string MuestroResultadoUnico(int[] distance, int verticesCount, int[] Etiqs, string[] caminos, int endingIndex, int startingIndex)
    {
        if (endingIndex <= verticesCount && distance[endingIndex] != int.MaxValue)
        {
            // Cantidad de Nodos para llegar:{distance[endingIndex].ToString()}\t
            return($"{caminos[endingIndex]}");
        }
        else
        {
            return("No se encontro un camino posible");
        }
    }
}
